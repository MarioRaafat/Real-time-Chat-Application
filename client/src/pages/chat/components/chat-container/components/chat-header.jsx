...
const ChatHeader = () => {
    const {closeChat, chatData, userInfo, chatType} = useAppstore();
            ...
            <div className="flex gap-3 items-center w-[50%] absolute left-8">
                <Avatar className="h-8 w-8 md:h-10 md:w-10 lg:h-12 lg:w-12 rounded-full overflow-hidden">
                    { image ? (
                        ...
                        <div
                            className={`font-bold text-l uppercase h-8 w-8 lg:h-12 lg:w-12  md:h-10 md:w-10 rounded-full flex items-center justify-center ${
                                chatType === "contact" ?
                                    getColor(color)
                                    :
                                    `bg-gray-400`
